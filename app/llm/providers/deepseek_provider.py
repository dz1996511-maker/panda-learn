...
class DeepSeekProvider(BaseLLMProvider):
    """DeepSeek API provider — uses OpenAI-compatible SDK with custom base URL."""
    ...
    async def chat(self, messages: list[LLMMessage], config: LLMConfig | None = None) -> LLMResponse:
        ...
        model = cfg.model or self.default_model or "deepseek-chat"
    ...
    async def embed(self, texts: list[str]) -> list[list[float]]:
        raise NotImplementedError(
            "DeepSeek provider does not support embeddings. "
            "Use OpenAI provider for embedding."
        )
